...
//-----------------------------------------
int encontraValorEspecifico(tlista l, tdado v){
	int i = 0;
	while (l.head != NULL){
		i++;
		if(l.head->dado == v)
...
printf("6- Remove\n");
printf("7-Procura de valor especifico\n");
printf("0-Sair\n");
scanf("%d",&op);
return op;
}
	...
	do{
       printList(l);
		...
		switch(op){
			...
			break;  
			        case 7: if(!isEmpty(&l)){
				    printf("Por favor, digite o valor que queira descobrir a posição : ");
				    scanf("%d",&v);
				    printf("\nValor: %d\nPosicao: %d\n",v,encontraValorEspecifico(l,v));
				    break;
			}         
